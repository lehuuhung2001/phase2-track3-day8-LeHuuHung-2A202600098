...
def render_report_stub(metrics: MetricsReport) -> str:  # noqa: PLR0914
    ...
    for m in metrics.scenario_metrics:
        ...
        rows.append(
            ...
            f"| {ok} | {m.nodes_visited} | {m.retry_count} | {m.interrupt_count} |"
        )
    table = "\n".join(rows)
    resume_status = "✅ PASS — SQLite crash-resume verified" if metrics.resume_success else (
        "⚠️ MemorySaver only (in-process persistence)"
    )
...
**success_rate = {metrics.success_rate:.0%} | total_scenarios = {metrics.total_scenarios}**
...
**Summary:**
- avg_nodes_visited: {metrics.avg_nodes_visited:.2f}
- total_retries: {metrics.total_retries} (S05 retried twice, S07 retried once then dead-lettered)
- total_interrupts: {metrics.total_interrupts} (S04 and S06 required approval)
...
`resume_success = {str(metrics.resume_success).upper()}` in `outputs/metrics.json`.
...
def write_report(metrics: MetricsReport, output_path: str | Path) -> None:
    """Write the full lab report to disk."""
